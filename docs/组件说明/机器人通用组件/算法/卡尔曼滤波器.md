...
$$
此处 $K\in [0, H^-]$ 即为卡尔曼增益，求取状态最优估计的问题转化为求取最优 $K$ 的问题。
...
\end{bmatrix}
$$
卡尔曼增益最优问题可转化为求 $K_k$ 使得 $\tr (P_k)$ 最小，思路为对 $\tr(P_k)$ 求导并令结果为 $0$ ，进而用其他量表示 $K_k$。以下进行求解。
...
$$
$\tr(P_k)$ 对 $K_k$ 求导，经计算得到：
$$
{\partial \tr (P_k) \over  \partial K_k}=-2P_k^{-T}H^T+2KHP_k^-H^T+2KR = 0
$$
则卡尔曼增益为：
$$
...
调用控制量设定方法进行动态设置（仅 KF_MODEL_BASED 模式可用）；调用预测方法，可获得状态量预测值，如：
...
| `xd, zd`                                          | `uint8_t`                 | `2, 2`                                   | 状态量和观测量的维度 |
| `x_x1d, P_xxd, Q_xxd, R_zzd, A_xxd, B_xxd, H_zxd` | `arm_matrix_instance_f32` | /                                        | 各种参数矩阵         |
...
| `setU` | （仅 KF_MODEL_BASED 类型）传入 `float*` u 控制量数组地址 | 设置状态控制量 |
