...
电机的电力学和动力学公式如下：
$$
\begin{align} u\left ( t \right ) &=Ri\left ( t \right )+L\frac{\partial i\left (t  \right ) }{\partial t}+e\\ J_{TM}\dot{\omega }   _M&=T_M-T_L \end{align}
$$
其中，u为等效输入电压，i为电机等效电流，L为电枢电感，R为电机等效内阻，e为电磁电压,  $$ J_{TM}$$为动力单元整体的转动惯量， $$ T_M$$为电机产生的总扭矩， $$T_L$$为总的载荷扭矩， $$\dot{\omega }   _M$$为电机的角加速度。

此外，电机扭矩和电机当前电流存在比例关系，电磁压力e与电机角速度也存在比例关系： 
$$
...
$$
\begin{align} U\left ( s \right ) &=\left ( R +Ls \right ) I\left ( s \right ) +K_1\Omega \left ( s \right )\\ I\left ( s \right )  &=\left ( K_2s+b \right ) \Omega \left ( s \right ) \end{align}
$$
则 $$U和I、I和\Omega$$间的传递函数为:
$$
\begin{align} \frac{I\left ( s \right ) }{U\left ( s \right ) } &= \frac{K_2s+b}{LK_2s^2+\left  (Lb+RK_2\right )s+Rb+K_1}\\ \frac{\Omega\left ( s \right ) }{I\left ( s \right ) } &= \frac{1}{K_2s+b} \end{align}
$$
综上可将期望电流的计算模型抽象为：
$$
...
1.3.0版功率限制代码，其主要核心思想为通过预测下一时刻的功率$$P_{pre}$$与当前计算的动态功率上限$$P_ref$$进行比较求出适当的速度削减系数$$k_{l}$$,等比例削减轮速，从而达到功率限制的效果。
...
加速度环控制系数$$k_a$$主要控制轮子可用的瞬时加速度（车辆的加速爆发能力），$$k_a$$值越小，车辆爆发加速越高，同时消耗在加速上的能量也越多。$$\color{red}{对于此值，如过度减小，会导致加速过程中的功率难以控制，从而出现在加速过程中发生超功率现象，所以不建议过度减小此值。}$$

$$\color{red}{并且加速过程容易出现打滑现象，过大的加速度并不利于能量的有效使用。}$$
