...
| 电机转矩电流目标值 | $i_{ref}(\rm mA)$        | 电机转矩电流目标值，基于转速目标值由控制器计算得出。C620 电调取值范围 $[-16384,16384]$，线性映射到 $[-20,20] \rm A$. 存储该值时单位定义为 $\rm mA$ |
...
使用转矩电流近似表示总电流，基于 $P_{loss} = R\cdot i^2$ 公式，在电机堵转的情况下统计 $i_q$ 和[功率估计](#功率估计)一节所述的融合估计结果 $\hat P$，以二阶模型拟合损耗曲线。模型为
...
\begin{split}
Predict: 
&\hat x_{k|k-1}=\rm \mathbb P_{model}\\
...
本算法**要求使用纯比例控制**计算电机的目标转矩电流 $i_{ref}$，各电机比例系数均为 $K_{pm}$：
...
其中 $a,b,c$ 参数为常量；由于 $k_{M}$ 并不能精确得到，需要根据上面滤波估计的实际功率，基于功率计算模型，不断进行修正：
...
算法中额外计算动态底盘功率上限 $P_{ref}$ （而不直接使用 $P_m$ 作为上限），是为了应对电机启动阶段瞬时功率较大的情况，此时功率限制应适当放宽，若以 $P_m$ 将难以启动。计算过程如下：
...
由功率预测模型，当预测底盘功率大于动态功率上限时，同比例削减各电机目标转速。设置削减系数 $k_l$，削减后电机 $\Omega_{ref}'=k_l\cdot\Omega_{ref}$ ，使得
...
$$
\begin{split}
\alpha = &aK_{p}^2 \sum_j  \Omega_{ref_j}^2\\
\beta = & (\hat k_{M}K_{p}-2aK_{p}^2 )\sum_j \Omega_j\Omega_{ref_j}+bK_{p}\sum_j \Omega_{ref_j}\\
\gamma= & (aK_{p}^2-\hat k_{M}K_{p}) \sum_j \Omega_j^2-bK_{p}\sum_j \Omega_j+\sum_j c-P_{ref}\\
\end{split}
$$
...
其中 `a_loss, b_loss, c_loss, PwrKfParams, FusionKfParams` 参数与算法原理相关，需要事先计算或整定。
...
PwrLimitRuntimeParams_t runtime_par = {
	...
	.iq_measure = iq_measure_,
	.spd_ref_rpm = spd_ref_rpm_,
	.iq_ref_ma = iq_ref_ma_,
	.kp_motor = <KP_MOTOR>
};
...
其中若底盘上连接了超电模块，则启用 `is_super_cap_online` 等参数（`SUPER_CAP_EXISTING` 宏语句范围内）。

然后调用方法解算限制后的各电机转速，存储于提供的数组中。

> 注意：最终得到的是与动态参数设置顺序对应的所有电机的目标转速，还需要自行使用控制器算法得到目标转矩电流，再发送给电调。
...
| `fusion_kf`      | `Kf_t`                    | /      | （仅当安装超电模块时启用）融合超级电容主控、裁判系统功率观测值的卡尔曼滤波器 |
...
| `a_loss, b_loss, c_loss` | `float`    | /      | 功率损耗相关参数，基于事先采集的数据由统计方法得到，具体参考算法原理 |
| `PwrKfParams`            | /          | /      | 融合功率模型计算值和观测值的卡尔曼滤波器参数                 |
| `FusionKfParams`         | /          | /      | （仅当安装超电模块时启用）融合超级电容主控、裁判系统功率观测值的卡尔曼滤波器参数 |
...
| `super_cap_mode`      | `PwrLimitCapMode_t` | `PWR_LIMIT_SUPER_CAP_OFF`<br />`PWR_LIMIT_SUPER_CAP_NORMAL`<br />`PWR_LIMIT_SUPER_CAP_NORMAL` | （仅当安装超电模块时启用）超级电容状态模式                   |
...
| `iq_measure`          | `int16_t*`          | /                                                            | 所有电机转矩电流量测的数组首地址，单位：C620电调反馈值，未定量纲 |
| `spd_ref_rpm`         | `int16_t*`          | /                                                            | 所有电机转速目标值的数组首地址，单位：$\rm rpm$              |
| `iq_ref_ma`           | `int16_t*`          | /                                                            | 所有电机转矩电流目标值的数组首地址，单位：$\rm mA$           |
| `kp_motor`            | `float`             | 10.0f                                                        | 底盘电机转速 P 控制的 $K_p$ 参数                             |
