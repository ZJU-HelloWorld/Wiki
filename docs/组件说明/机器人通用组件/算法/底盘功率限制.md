...
> 根据手册，转矩常数为 $0.3\  \rm N\cdot m/A$，但该数据对应线电流，因此并不能直接使用；但可以经过量纲转换，作为 $k_M$ 的初值参考。

使用转矩电流近似表示线电流，基于 $P_{loss} = R\cdot i^2$，在电机堵转的情况下统计 $i_q$ 和功率观测 $P_{measure}$ ，拟合损耗曲线。以 C620 电调 + M3508 电机为例，堵转电机，在 2000ms 内以 $[0:16384/2000:16384]$ 扫描转矩电流，$P-i$ 曲线拟合结果如下（供参考）：
...
本算法**要求使用纯比例控制**计算电机的目标转矩电流 $i_{ref}$，按照各电机的 $PID$ 控制器参数，比例系数分别为 $K_{p_j}$，同时进行限幅：

$$
i_{ref_j}={\rm LIMIT\_MAX}\ [K_{p_j}(\Omega_{ref_j}-\Omega_j)]
$$
...
需要注意，裁判系统无法量测负功率，因此当估计功率为负时，不应更新 $\hat k_M$。
...
由功率预测模型，当预测出的底盘功率大于动态功率上限时，同比例削减各电机目标转速。设置削减系数 $k_l$，削减后电机 $\Omega_{ref}'=k_l\cdot\Omega_{ref}$ .

首先处理存在电机输出电流超出控制器限幅的情况，记限制系数为 $k_{0}$，输出上限为 $U_m$:

$$
\Omega_{ref_j}'={U_{m_j}\over K_{p_j}}+\Omega_j\Rightarrow k_{0_j}={{U_{m_j}}+K_{p_j}\cdot \Omega_j \over K_{p_j} \cdot \Omega_{ref_j}}
$$

若电机未超限则 $k_{0_j}=1$. 随后求取 $k_1$，使得

$$
\hat k_{M}\sum_j \Omega_j \cdot K_{p}( k_1\cdot k_{0_j} \Omega_{ref_j}-\Omega_j)+
\sum_j 
\hat R \cdot K_{p}^2 (k_1 \cdot k_{0_j}\Omega_{ref_j}-\Omega_j)^2
+P_0 \triangleq P_{ref}
$$
...
$$
\begin{split}
\alpha = &\hat R K_{p}^2 \sum_j k_{0_j}^2 \Omega_{ref_j}^2\\
\beta = & (\hat k_{M}K_{p}-2\hat RK_{p}^2 )\sum_j k_{0_j}\Omega_j\Omega_{ref_j}\\
\gamma= & (\hat RK_{p}^2-\hat k_{M}K_{p}) \sum_j \Omega_j^2+P_0-P_{ref}\\
\end{split}
$$
...
$$
k_1=\frac{-\beta\pm\sqrt{\beta^2-4\alpha\gamma}}{2\alpha},取解\in (0,1)
$$
