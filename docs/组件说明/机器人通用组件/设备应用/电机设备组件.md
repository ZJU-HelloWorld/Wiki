...
由于队内使用的电机均通过 CAN 进行命令的发送与反馈数据的接收（使用 RS485 的电机通过外接自制 CAN 与 RS485 的转换板进行通信）。同时，不同的电机在使用时绝大部分的功能是相同的，同时机器人上常常有不同种类混用的情况，因此根据电机的共有特点，抽象出一个电机基类，其包含基本的电机操作，通过指向具体电机实例的基类指针进行电机操作将带来极大的便利性，同时，当有更换电机种类时，也可实现极少代码的修改实现迁移。作为基类应具有一下基本功能：
...
实例化一个电机组件并放入对应电机参数进行初始化，建议再初始化后即使将其添加到 CAN 或 FDCAN 的发送与接收管理器中，如：
