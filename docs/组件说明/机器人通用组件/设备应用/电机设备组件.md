...
|`torq_input_lim`|`float`|10|电机输出端限制力矩值，单位：N·m|
|`curr_input_lim`|`float`|10|电机电流输入限制值，单位：A|
|`torq_const`|`float`|0.7|力矩常数，单位：N·m/A|
...
|`setAngleValue`|`angle`: 指定角度，单位：rad|将当前电机输出端所处位置设置为指定的角度|
...
|`raw2torq`|`raw`: 原始报文数值</br>返回: 原始报文对应的输出端力矩值，单位：N·m|将原始报文内容转换为输出端力矩|
|`torq2raw`|`torq`: 输出端力矩值，单位：N·m</br>返回: 输出端力矩值对应的原始报文|将输出端力矩转换为原始报文内容|
|`raw2curr`|`raw`: 原始报文数值</br>返回: 原始报文对应的转子电流值，单位：A|将原始报文内容转换为转子电流|
|`curr2raw`|`curr`: 转子电流值，单位：A</br>返回: 转子电流值对应的原始报文|将转子电流转换为原始报文内容|
|`torq2curr`|`torq`: 输出端力矩值，单位：N·m</br>返回: 输出端力矩值对应的转子电流，单位：A|将输出端力矩转换为转子电流|
|`curr2torq`|`curr`: 转子电流值，单位：A</br>返回: 转子电流值对应的输出端力矩，单位：N·m|将转子电流转换为输出端力矩|
|`motor_info`|/|/|
|`dir`|/|/|
|`id`|/|/|
|`rx_id`|/|/|
...
| 名称                    | 类型            | 示例值      | 描述                                                      |
| :---------------------- | :-------------- | :---------- | :-------------------------------------------------------- |
|`angle_`|`float`|/|电机输出端角度，单位：rad|
|`vel_`|`float`|/|电机输出单角速度，单位：rad/s|
|`torq_`|`float`|/|电机输出端实际力矩，单位：N·m|
|`curr_`|`float`|/|电机转子实际电流，单位：A|
|`round_`|`float`|/|电机转子端累计圈数|
|`last_raw_angle_`|`float`|/|电机上次角度原始值|
|`actual_angle_`|`float`|/|电机输出端实际输出角度（未扣除角度偏差），单位：rad|
...
| 名称             | 参数说明                                                              | 描述                                                                                                 |
| :--------------- | :-------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
|`normAngle`|`angle`: 待转换的角度，单位：rad</br>返回: 归一化后的角度，单位：rad|将角度转换到指定范围|
