...
模块化组件在`HW-Components`仓库中的`cpp25-dev-module_feat`分支，其中`modules`文件夹使用`git submodule`管理。
...
`Feed::StuckStatus`枚举类用于表示拨弹盘当前的卡弹状态，各枚举值的含义与命名一致。
...
```C++
if (feed_ptr->getStuckStatus() == hello_world::module::Feed::StuckStatus::Forward) {
  /* do something */
}
```

### 1.2 拨弹盘所需的裁判系统反馈数据（结构体）<a id="1.2"></a>
...
`Feed`类的正常运行需要实时获取裁判系统的反馈数据，`Feed`类提供的接口如下
...
  float resurrection_pos_err        = 5 / 180 * PI;  // Resurrection 模式反转速度系数, 反转时以它作为位置环PID的误差值, >0, 单位 rad
...
|   `resurrection_pos_err`   | `5 / 180 * PI` | 拨弹盘是通过双环`PID`（位置环+速度环）进行控制的。为实现拨弹盘在复活模式下匀速反转，控制时，将`PID`位置环的误差值始终设为`-resurrection_pos_err`。                                                                                                                                                                                                       | 它仅与复活模式下拨弹盘的反转速度有关，并且成正比。                                                                                                                               |
...
**【要求】每个控制周期**，用户都需要使用[#2.1 拨弹盘的实例化与初始化](#2.1)创建的实例，按照顺序调用以下接口函数，更新数据，设定控制指令。
...
1. `updateRfrData()`用于更新拨弹盘所需的裁判系统反馈数据，不更新时，采用用户上一次的更新值或者默认值。（详见[#1.2 拨弹盘所需的裁判系统反馈数据（结构体）](#1.2)）
...
重载了三个函数来分别实现 **禁止拨弹**；**允许拨弹，限制频率但不限制热量**；**允许拨弹，限制频率和热量**。

```c++
void setTriggerLimit(bool is_trigger_allowed);
void setTriggerLimit(uint32_t interval, bool is_heat_limited, bool is_trigger_allowed = true);
void setTriggerLimit(uint32_t interval, float safe_num, bool is_heat_limited = true, bool is_trigger_allowed = true);
```
...
```c++
hello_world::module::Feed* feed_ptr_ = CreateFeed();
feed_ptr_->setTriggerLimit(false);       // 禁止拨弹
feed_ptr_->setTriggerLimit(100, false);  // 允许拨弹，限制拨弹间隔 100ms，不限制热量
feed_ptr_->setTriggerLimit(100, 1.5);    // 允许拨弹，限制拨弹间隔 100ms，限制安全热量对应的弹丸颗数为 1.5 颗
```
...
1. 同时设定两个发射指令时，拨弹盘根据控制模式`ctrl_mode_`是` hello_world::module::CtrlMode::Manual`还是`hello_world::module::CtrlMode::Auto`来决定听从操作手发弹信号还是视觉指令发弹信号。
2. 拨弹盘在每次控制周期内，处理完这两个发弹信号后，会将它们主动复位，以防止用户在设定一次发弹信号后不设定回`false`导致一直发弹的情况。
