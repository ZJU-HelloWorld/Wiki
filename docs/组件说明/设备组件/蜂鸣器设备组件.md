...
指定每个音符的时长和整体强度 $\in (0,1]$（数值太小可能无法正常播放），编写需要播放的音符列表（类型 `Tune_e`，音符表示如：`A4_` 基本音，` A4S` 升，` A4F` 降，`RST` 休止），以 `EOL` (end of list) 结尾，例如：
...
```c
buzzer.beepStart(&buzzer, &kWarningList1B, BEEP_ONCE); // 播放一次

buzzer.beepStart(&buzzer, &kWarningList1B, BEEP_LOOP); // 循环播放
```

在播放过程中，需要不断调用 `beepCtrlHook` 方法来控制音符时长和切换音调，调用频率至少为 1kHz。例如在一个 1KHz 的定时器中断回调函数中：
...
单次播放完毕后将自动失能蜂鸣器。循环播放时，则需要手动调用 `mute` 方法失能蜂鸣器。
...
| `is_loop`          | `BeepType_e`         | BEEP_LOOP <br> BEEP_ONCE  | 播放类型设置   |
...
| `mute`               | /                                                        | 给干沉默了。                                                 |
| `beepStart`          | 传入音符列表结构体指针 `p_tune_list`，包含了播放相关信息；`beep_config` 指定播放类型设置 | 根据传入信息开始播放。                 |
| `beepCtrlHook`       | /                                                        | 控制音符时长和切换音调的钩子函数，需在播放过程中以 1KHz 以上频率被调用。 |
...
| `list`                     | `Tune_e[TUNE_LIST_MAX_LEN]` | /      | 音符列表，以 `EOL` 结尾；音符的表示方式参考 `Tune_e` |
